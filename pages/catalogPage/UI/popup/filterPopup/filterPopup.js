document.addEventListener('DOMContentLoaded', function () {
    ...
    const mainFilters = [
        {
            ...
            backgroundImage: '/static/images/interior__1.png',
            name: 'Аренда жилья'
        },
    ...
    let mainBlockFilters = []
    let commercialBlockFilters = []

    function updateFilterCount() {
        const totalFilters = mainBlockFilters.length + commercialBlockFilters.length
        document.getElementById('applyBtnText').textContent = `Применить ${totalFilters > 0 ? `(${totalFilters})` : ''}`
        document.getElementById('applyBtnTextBefore').textContent = `Применить ${totalFilters > 0 ? `(${totalFilters})` : ''}`
        document.getElementById('mainFilterAmount').textContent = `${mainBlockFilters.length ? `(${mainBlockFilters.length})` : ''}`
        document.getElementById('commercialFilterAmount').textContent = `${commercialBlockFilters.length ? `(${commercialBlockFilters.length})` : ''}`
    }

    function toggleFilter(filter, block) {
        const filterIndex = block.findIndex(f => f.id === filter.id)
        if (filterIndex > -1) {
            block.splice(filterIndex, 1)
        } else {
            block.push(filter)
        }
        updateFilterCount()
    ...
    function renderFilterCards() {
        const mainFilterCardsContainer = document.getElementById('mainFilterCardsContainer')
        const commercialFilterCardsContainer = document.getElementById('commercialFilterCardsContainer')

        mainFilters.forEach(filter => {
            let card = mainFilterCardsContainer.querySelector(`[data-filter-id="${filter.id}"]`)
            ...
            if (!card) {
                ...
                card.innerHTML = `
                    <img src="/static/images/interior__6.png" alt="${filter.name}" />
                    <div class="filter__card__ui__container">
                        <div class="filter__card__icon">${filter.icon}</div>
                        <span class="filter__name">${filter.name}</span>
                    </div>
                `
                card.addEventListener('click', () => toggleFilter(filter, mainBlockFilters))
                mainFilterCardsContainer.appendChild(card)
            }

            if (mainBlockFilters.some(f => f.id === filter.id)) {
        ...
        commercialFilters.forEach(filter => {
            let card = commercialFilterCardsContainer.querySelector(`[data-filter-id="${filter.id}"]`)
            ...
            if (!card) {
                ...
                card.setAttribute('data-filter-id', filter.id)
                card.innerHTML = `
                    <img src="${filter.backgroundImage}" alt="${filter.name}" />
                    <div class="filter__card__ui__container">
                        <div class="filter__card__icon">${filter.icon}</div>
                        <span class="filter__name">${filter.name}</span>
                    </div>
                `
                card.addEventListener('click', () => toggleFilter(filter, commercialBlockFilters))
                commercialFilterCardsContainer.appendChild(card)
            }

            if (commercialBlockFilters.some(f => f.id === filter.id)) {
                card.classList.add('active')
                card.classList.remove('not-active')
            } else {
                card.classList.add('not-active')
    ...
    document.getElementById('applyBtn').addEventListener('click', () => {
        console.log('Filters Applied:', { mainBlockFilters, commercialBlockFilters })
        alert('Filters applied!')
    })

    document.getElementById('cancelBtn').addEventListener('click', () => {
        mainBlockFilters = []
        commercialBlockFilters = []
        updateFilterCount()
        renderFilterCards()
        closePopup()
    })
